...
FASTER_RCNN_LABEL_NAMES = ["bottom", "middle", "top"]

def load_faster_rcnn(model_path: str) -> FRCNN:
...
def predict_faster_rcnn(image: Image.Image, model: FRCNN, conf: float = 0.25, iou: float = 0.45) -> Tuple[Image.Image, Dict]:
    """
    使用 Faster R-CNN 执行推理并返回统一格式的结果
    ...
    """
    # 导入必要的工具
    from utils.utils import cvtColor, get_new_img_size, resize_image, preprocess_input
    import torch
    ...
    image_data = resize_image(image_rgb, [input_shape[1], input_shape[0]])
    ...
    with torch.no_grad():
        ...
        results = model.bbox_util.forward(
            roi_cls_locs, roi_scores, rois, image_shape, input_shape,
            nms_iou=iou, confidence=conf  # 使用传入的参数
        )
    ...
    if len(results[0]) > 0:
        ...
        # 统计各类别数量（0=bottom, 1=middle, 2=top）
        for i in range(3):
    ...
    if len(results[0]) > 0:
        ...
        font = ImageFont.truetype(
            font=os.path.join(LOAD_MODELS_DIR, 'model_data/msYahei.ttf'),
...
def load_model(pt_path: str, yaml_path: Optional[str] = None):
    """统一的模型加载接口"""
    return load_faster_rcnn(pt_path)


def predict(image: Image.Image, model, conf: float = 0.25, iou: float = 0.45):
    """统一的预测接口"""
    return predict_faster_rcnn(image, model, conf=conf, iou=iou)